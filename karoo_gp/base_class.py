...
class BaseGP(BaseEstimator):
    ...
    def __init__(
        ...
        self._pop_initialized = False        # flag for population init in fit
        ...
        # TODO support function type as args, e.g. 'logic, arithmetic'
            ...
            raise ValueError('functions must be a list of strings or type '
                             'Functions')
        # Validate Terminals and Constants
        # `terminals` must be a list of strings, and it must match the columns
        # of the X passed to fit. In some cases (e.g. sklearn Pipeline), the
        # columns are transformed by the pipeline, after this has initialized,
        # which changes the columns of X. So terminals should be generated
        # based on X (in fit) and trees after that. If labels are provided
        # here, they must match the shape of X. If not provided here, they are
        # autogenerated.
        if terminals is not None and (
            not isinstance(terminals, list) or
            not all(isinstance(t, str) for t in terminals)):
            raise ValueError('terminals must be a list of strings')
        self._terminals = terminals
        if constants is not None and (
            not isinstance(constants, list) or
            not all(type(c) in [int, float] for c in constants)):
    ...
    def fit(self, X=None, y=None):
        ...
        if not self._pop_initialized:
            # Initialize terminals
            if self._terminals is None:
                self._terminals = [f'f{i}' for i in range(X.shape[1])]
            elif len(self._terminals) != X.shape[1]:
                raise ValueError('Number of terminals must match number '
                                 'of features')
            self.terminals = Terminals(self._terminals, self._constants)
