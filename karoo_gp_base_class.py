...
np.set_printoptions(
    linewidth=320)  # set the terminal to print 320 characters before line-wrapping in order to view Trees
...
class Base_GP(object):
    ...
    def __init__(self):
        ...
        self.core_count = -1  # sklearn format
    ...
    def karoo_gp(self, tree_type, tree_depth_base, filename):
        ...
        self.population_a = ['Karoo GP by Kai Staats, Generation ' + str(
            self.generation_id)]  # a list which will store all Tree arrays, one generation at a time
        ...
        for self.generation_id in range(2, self.generation_max + 1):  # loop through 'generation_max'
            ...
            self.population_a = self.fx_evo_pop_copy(self.population_b,
                                                     ['GP Tree by Kai Staats, Generation ' + str(self.generation_id)])
    ...
    def fx_karoo_data_load(self, tree_type, tree_depth_base, filename):
        ...
        data_dict = {'b': 'files/data_BOOL.csv', 'c': 'files/data_CLASSIFY.csv', 'r': 'files/data_REGRESS.csv',
                     'm': 'files/data_MATCH.csv', 'p': 'files/data_PLAY.csv'}
        ...
        func_dict = {'b': 'files/functions_BOOL.csv', 'c': 'files/functions_CLASSIFY.csv',
                     'r': 'files/functions_REGRESS.csv', 'm': 'files/functions_MATCH.csv',
                     'p': 'files/functions_PLAY.csv'}
        ...
        self.algo_ops = sp.symbols(
            self.terminals)  # convert a string of terminals to sympy executables - tested 2016 08/29

        self.class_labels = len(
            np.unique(data_y))  # load the user defined labels for classification or solutions for regression
        self.coeff = np.loadtxt('files/coefficients.csv', delimiter=',', skiprows=1,
                                dtype=str)  # load the user defined coefficients (constants)
        ...
        for row in range(0, self.data_train_rows):  # increment through each row of data
            for col in range(0, self.data_train_cols):  # increment through each column
                data_train_dict.update(
                    {self.terminals[col]: data_train[row, col]})  # to be unpacked in 'fx_fitness_eval'
    ...
    def fx_karoo_point_mutate(self):
        ...
        for n in range(self.evolve_point):  # quantity of Trees to be generated through mutation
            ...
            tourn_winner, node = self.fx_evo_point_mutate(
    ...
    def fx_karoo_crossover(self):
        ...
        for n in range(
            ...
            branch_a = self.fx_evo_branch_select(
            ...
            branch_b = self.fx_evo_branch_select(
    ...
    def fx_karoo_pause(self, eol):
        ...
        options = ['?', 'help', 'i', 'm', 'g', 's', 'db', 't', 'ts', 'min', 'max', 'b', 'c', 'id', 'pop', 'l', 'p', 'a',
                   'test', 'cont', 'load', 'w', 'q', '']
        ...
        while True:
            try:
                ...
                if pause == '':
                    if eol == 1:
                        self.fx_karoo_pause(1)  # return to pause menu as the GP run is complete
                    else:
                        break  # drop back into the current GP run
                ...
                if pause == '?' or pause == 'help':
                    ...
                    if eol == 1:
                        print '\t\033[0;0m Remember to archive your final population before your next run!'
                    else:
                        print '\t\033[36m\033[1m ENTER\033[0;0m to continue ...'

                elif pause == 'i':
                    self.display = 'i';
                    print '\t Interactive mode engaged (for control freaks)'
                elif pause == 'm':
                    self.display = 'm';
                    print '\t Minimal mode engaged (for recovering control freaks)'
                elif pause == 'g':
                    self.display = 'g';
                    print '\t Generation mode engaged (for GP gurus)'
                # elif pause == 's': self.display = 's'; print '\t Server mode engaged (for zen masters)'
                elif pause == 'db':
                    self.display = 'db';
                    print '\t Debug mode engaged (for vouyers)'
                elif pause == 't':
                    self.display = 't';
                    print '\t Timer mode engaged (for managers)'


                elif pause == 'ts':  # adjust the tournament size

                    menu = range(2,
                                 self.tree_pop_max + 1)  # set to total population size only for the sake of experimentation
                    while True:
                        try:
                            ...
                            self.tourn_size = query;
                            break
                        except ValueError:
                            print '\n\t\033[32m Enter a number from 2 including', str(
                                self.tree_pop_max) + ".", 'Try again ...\033[0;0m'
                    ...
                    while True:
                        try:
                            ...
                            if query not in str(menu):
                                raise ValueError()
                            elif query == '':
                                break
                            self.evolve_repro = int(float(query) / 100 * self.tree_pop_max);
                            break
                        except ValueError:
                    ...
                    while True:
                        try:
                            ...
                            if query not in str(menu):
                                ...
                                break
                            self.evolve_point = int(float(query) / 100 * self.tree_pop_max);
                            break
                        except ValueError:
                            print '\n\t\033[32m Enter a number from 0 including 100. Try again ...\033[0;0m'
                    ...
                    while True:
                        try:
                            ...
                            if query not in str(menu):
                                ...
                                break
                            self.evolve_branch = int(float(query) / 100 * self.tree_pop_max);
                            break
                        except ValueError:
                            print '\n\t\033[32m Enter a number from 0 including 100. Try again ...\033[0;0m'
                    ...
                    while True:
                        try:
                            ...
                            if query not in str(menu):
                                ...
                                break
                            self.evolve_cross = int(float(query) / 100 * self.tree_pop_max);
                            break
                        except ValueError:
                            print '\n\t\033[32m Enter a number from 0 including 100. Try again ...\033[0;0m'
                    ...
                    menu = range(1,
                                 self.core_count + 1)  # assuming any run above 24 cores will simply use the maximum number
                    while True:
                        try:
                            print '\n\t The current number of engaged CPU cores is:', self.cores
                            query = int(raw_input('\n\t Adjust the number of CPU cores (min 1): '))
                            if query not in (menu):
                                raise ValueError()
                            elif query == '':
                                break
                            self.cores = int(query);
                            break
                        except ValueError:
                            print '\n\t\033[32m Enter a number from 1 including', str(
                                self.core_count) + ".", 'Try again ...\033[0;0m'
                    ...
                    if self.generation_id == 1:
                        for tree_id in range(1, len(self.population_a)):
                            ...
                            print '\t\033[36m Tree', self.population_a[tree_id][0][
                                1], 'yields (sym):\033[1m', self.algo_sym, '\033[0;0m'
                        ...
                        for tree_id in range(1, len(self.population_b)):
                            ...
                            print '\t\033[36m Tree', self.population_b[tree_id][0][
                                1], 'yields (sym):\033[1m', self.algo_sym, '\033[0;0m'
                    ...
                    if self.generation_id == 1:
                        ...
                        while True:
                            try:
                                ...
                                if query not in str(menu) or query == '0':
                                    raise ValueError()
                                elif query == '':
                                    break
                                self.fx_eval_tree_print(self.population_a[int(query)]);
                                break
                            except ValueError:
                                print '\n\t\033[32m Enter a number from 1 including', str(
                                    len(self.population_a) - 1) + ".", 'Try again ...\033[0;0m'
                        ...
                        while True:
                            try:
                                ...
                                if query not in str(menu) or query == '0':
                                    ...
                                    break
                                self.fx_eval_tree_print(self.population_b[int(query)]);
                                break
                            except ValueError:
                                print '\n\t\033[32m Enter a number from 1 including', str(
                                    len(self.population_b) - 1) + ".", 'Try again ...\033[0;0m'
                        ...
                        while True:
                            try:
                                ...
                                if query not in str(menu) or query == '0':
                                    ...
                                    break
                                if self.kernel == 'b':
                                    self.fx_test_bool(int(query));
                                    break
                                elif self.kernel == 'c':
                                    self.fx_test_classify(int(query));
                                    break
                                elif self.kernel == 'r':
                                    self.fx_test_regress(int(query));
                                    break
                                elif self.kernel == 'm':
                                    self.fx_test_match(int(query));
                                    break
                    ...
                    while True:
                        try:
                            ...
                            self.fx_karoo_continue(
                                next_gen_start)  # continue evolving, starting with the last population
                        except ValueError:
                    ...
                    while True:
                        try:
                            ...
                            if query == 'y':
                                self.fx_karoo_data_recover(self.filename['s']);
                                break
                            elif query == 'n':
                                break
                        except ValueError:
                    ...
                    if eol == 0:
                        ...
                        query = raw_input('\n\t Are you certain you want to quit? (y/n)')
                        if query == 'y':
                            print '\n\t \033[32mRemember to archive the Trees found in karoo_gp/files/ before your next run ...\033[0;0m';
                            sys.exit()
                        else:
                            self.fx_karoo_pause(1)

            except ValueError:
                print '\t\033[32m Select from the options given. Try again ...\033[0;0m'
    ...
    def fx_karoo_continue(self, next_gen_start):
        ...
        for self.generation_id in range(next_gen_start,
                                        self.generation_max + 1):  # evolve additional generations of Trees
    ...
    def fx_gen_root_node_build(self):
        ...
        if self.pop_node_arity == 1:  # 1 child
            ...
            print '\n\t\033[31mERROR! In fx_gen_root_node_build: pop_node_arity =', self.pop_node_arity, '\033[0;0m';
            self.fx_karoo_pause(
                0)
            ...
            for j in range(1, len(self.tree[3])):  # increment through all nodes (exclude 0) in array 'tree'
                ...
                if int(self.tree[4][j]) == self.pop_node_depth - 1:  # find parent nodes which reside at the prior depth
                    ...
                    for k in range(1, int(
                        ...
                        prior_sibling_arity = self.fx_gen_function_gen(parent_arity_sum, prior_sibling_arity,
    ...
    def fx_gen_function_gen(self, parent_arity_sum, prior_sibling_arity, prior_siblings):
        ...
        if self.pop_tree_type == 'f':  # user defined as (f)ull
            ...
            if rnd == 0:  # randomly selected as Function
                self.fx_gen_function_select()  # retrieve a function
                self.fx_gen_child_link(parent_arity_sum, prior_sibling_arity,
    ...
    def fx_gen_terminal_select(self):
        ...
        rnd = np.random.randint(0,
                                len(self.terminals) - 1)  # call the previously loaded .csv which contains all terminals
        ...
        for n in range(1, len(self.tree[3])):  # increment through all nodes (exclude 0) in array 'tree'
            ...
            if int(self.tree[4][
                ...
                if self.pop_node_arity == 0:  # terminal in a Grow Tree
                    ...
                    print '\n\t\033[31mERROR! In fx_gen_child_link: pop_node_arity =', self.pop_node_arity, '\033[0;0m';
                    self.fx_karoo_pause(
                        0)
        ...
        for depth in range(0,
            ...
            for node in range(1, len(tree[3])):  # increment through all nodes (redundant, I know)
                if int(tree[4][node]) == depth:
                    ...
                    print ind, '  arity:', tree[8][node], '\tchild node(s):', tree[9][node], tree[10][node], tree[11][
                        node]
    ...
    def fx_eval_generation(self):
        ...
        self.fx_tree_archive(self.population_b,
                             'a')  # archive the current, evolved generation of Trees as the next foundation population
    ...
    def fx_fitness_gym(self, population):
        ...
        for tree_id in range(1, len(population)):
            ...
            if self.display not in ('s', 't'): print '\t\033[36mTree', population[tree_id][0][
                1], 'yields (sym):\033[1m', self.algo_sym, '\033[0;0m'
            ...
            if self.cores == 1:  # employ only one CPU core and bypass 'pprocess' to avoid overhead
                ...
                # serialize class for use in parallelize calculations
                temp_file = tempfile.mktemp()
                joblib.dump(self, temp_file)
                # high overhead so send chunks of processes
                chunks = int(self.data_train_rows / self.cores)
                results = joblib.Parallel(n_jobs=self.cores)(
                    joblib.delayed(paralell_fit_func)(tree_id, start, chunks, self.data_train_rows, temp_file) for start
                    in xrange(0, self.data_train_rows, chunks))
                # reduce    
                fitness = sum(results)
            ...
            if self.display == 't':
                print '\t \033[36m The fitness_gym has an ellapsed time of \033[0;0m\033[31m%f\033[0;0m' % (
                    time.time() - start), '\033[36mon', self.cores, 'cores\033[0;0m'
        ...
        if self.display == 't': print '\n\t \033[36m The sum of all Tree timings is \033[0;0m\033[31m%f\033[0;0m' % time_sum, '\033[0;0m'; self.fx_karoo_pause(
            0)

        print '\n\033[36m ', len(self.fittest_dict.keys()), 'trees\033[1m', np.sort(
            self.fittest_dict.keys()), '\033[0;0m\033[36moffer the highest fitness scores.\033[0;0m'
    ...
    def fx_fitness_eval(self, tree_id, row):
        ...
        result = self.fx_eval_subs(tree_id, self.data_train_dict_array[
            row])  # process the expression against the training data - tested 2016 07

        # if self.population_a[tree_id][12][3] == 'error': fitness = 0
        # else: # a place holder for future treatmemt of trees with errors

        solution = round(float(self.data_train_dict_array[row]['s']),
                         self.precision)  # force 'solution' to the set number of floating points
    ...
    def fx_fitness_function_classify(self, row, result, solution):
        ...
        if solution == 0 and result <= 0 - skew:  # check for first class (the left-most bin)
            if self.display == 'i': print '\t\033[36m data row', row, 'yields class label:\033[1m', int(
                solution), 'as', result, '<=', int(0 - skew), '\033[0;0m'
            fitness = 1

        elif solution == self.class_labels - 1 and result > solution - 1 - skew:  # check for last class (the right-most bin)
            if self.display == 'i': print '\t\033[36m data row', row, 'yields class label:\033[1m', int(
                solution), 'as', result, '>', int(solution - 1 - skew), '\033[0;0m'
            fitness = 1

        elif solution - 1 - skew < result <= solution - skew:  # check for class bins between first and last
            if self.display == 'i': print '\t\033[36m data row', row, 'yields class label:\033[1m', int(
                solution), 'as', int(solution - 1 - skew), '<', result, '<=', int(solution - skew), '\033[0;0m'
            fitness = 1
    ...
    def fx_fitness_tournament(self, tourn_size):
        ...
        for n in range(tourn_size):
            ...
            if self.fitness_type == 'max':  # if the fitness function is Maximising
                ...
                # tourn_test remains unchanged
                    ...
                    print '\n\t\033[31mERROR! In fx_fitness_tournament: fitness =', fitness, 'and tourn_test =', tourn_test, '\033[0;0m';
                    self.fx_karoo_pause(
                        0)
        ...
        if self.display == 'i': print '\n\t\033[36mThe winner of the tournament is Tree:\033[1m', tourn_winner[0][
            1], '\033[0;0m'
    ...
    def fx_fitness_gene_pool(self):
        ...
        for tree_id in range(1, len(self.population_a)):
            ...
            if len(self.population_a[tree_id][
                       3]) - 1 >= self.tree_depth_min and self.algo_sym != 1:  # if Tree meets the requirements
        ...
        if len(self.gene_pool) > 0 and self.display == 'i':
            ...
            print "\n\t\033[31m\033[3m 'They're dead Jim. They're all dead!'\033[0;0m There are no Trees in the gene pool. You should archive your populations and (q)uit.";
            self.fx_karoo_pause(0)
    ...
    def fx_evo_point_mutate(self, tree):
        ...
        if self.display == 'i':
            print '\t\033[36m with', tree[5][node], 'node\033[1m', tree[3][
                node], '\033[0;0m\033[36mchosen for mutation\n\033[0;0m'
        elif self.display == 'db':
        ...
        if tree[5][node] == 'root':
            ...
            print '\n\t\033[31mERROR! In fx_evo_point_mutate, node_type =', tree[5][
                node], '\033[0;0m';
            self.fx_karoo_pause(0)
    ...
    def fx_evo_full_mutate(self, tree, branch):
        ...
        for n in range(len(branch)):
            ...
            if tree[5][branch[n]] == 'func':
                if self.display == 'i': print '\t\033[36m  from\033[1m', tree[5][
                    branch[n]], '\033[0;0m\033[36mto\033[1m func \033[0;0m'
                ...
                if self.display == 'i': print '\t\033[36m  from\033[1m', tree[5][
                    branch[n]], '\033[0;0m\033[36mto\033[1m term \033[0;0m'
    ...
    def fx_evo_grow_mutate(self, tree, branch):
        ...
        branch_depth = self.tree_depth_max - int(tree[4][
                                                     branch_top])  # 'tree_depth_max' - depth at 'branch_top' to set max potential size of new branch - 2016 07/10
        ...
        if branch_depth < 0:  # this has never occured ... yet
            ...
            if self.display == 'i': print '\t\033[36m max depth branch node\033[1m', tree[3][
                branch_top], '\033[0;0m\033[36mmutates from \033[1mterm\033[0;0m \033[36mto \033[1mterm\033[0;0m\n'
            ...
            if type_mod == 'term':  # mutate 'branch_top' to a terminal and delete all nodes beneath (no subsequent nodes are added to this branch)
                ...
                if self.display == 'i': print '\t\033[36m branch node\033[1m', tree[3][
                    branch_top], '\033[0;0m\033[36mmutates from\033[1m', tree[5][
                    branch_top], '\033[0;0m\033[36mto\033[1m term \n\033[0;0m'
            ...
            if type_mod == 'func':  # mutate 'branch_top' to a function (a new 'gp.tree' will be copied, node by node, into 'tourn_winner')
                ...
                if self.display == 'i': print '\t\033[36m branch node\033[1m', tree[3][
                    branch_top], '\033[0;0m\033[36mmutates from\033[1m', tree[5][
                    branch_top], '\033[0;0m\033[36mto\033[1m func \n\033[0;0m'
                if self.display == 'db': print '\n\n\033[33m *** Grow Mutation: branch_top to function *** \033[0;0m\n\n\033[36m This is the unaltered tourn_winner:\033[0;0m\n', tree

                self.fx_gen_tree_build('mutant', self.pop_tree_type,
                                       branch_depth)  # build new Tree ('gp.tree') with a maximum depth which matches 'branch'
                ...
                tree = self.fx_evo_branch_top_copy(tree,
                                                   branch)  # copy root of new 'gp.tree' to point of mutation ('branch_top') in 'tree' ('tourn_winner')
                tree = self.fx_evo_branch_body_copy(
                    tree)  # copy remaining nodes in new 'gp.tree' to 'tree' ('tourn_winner')
    ...
    def fx_evo_crossover(self, parent, branch_x, offspring, branch_y):
        ...
        if len(branch_x) == 1:  # if the branch from the parent contains only one node (terminal)
            ...
            if self.display == 'i': print '\t\033[36m  terminal crossover from \033[1mparent', parent[0][
                1], '\033[0;0m\033[36mto \033[1moffspring', offspring[0][
                1], '\033[0;0m\033[36mat node\033[1m', branch_top, '\033[0;0m'
            ...
            offspring[6][branch_top] = parent[6][
                crossover]  # replace label with that of a particular node in 'branch_x'
            offspring[8][branch_top] = 0  # set terminal arity

            offspring = np.delete(offspring, branch_y[1:],
                                  axis=1)  # delete all nodes beneath point of mutation ('branch_top')
            ...
            if self.display == 'i': print '\t\033[36m  branch crossover from \033[1mparent', parent[0][
                1], '\033[0;0m\033[36mto \033[1moffspring', offspring[0][
                1], '\033[0;0m\033[36mat node\033[1m', branch_top, '\033[0;0m'

            # self.fx_gen_tree_build('test', 'f', 2) # TEST & DEBUG: disable the next 'self.tree ...' line
            self.tree = self.fx_evo_branch_copy(parent,
            ...
            offspring = self.fx_evo_branch_top_copy(offspring,
                                                    branch_y)  # copy root of 'branch_y' ('gp.tree') to 'offspring'
            offspring = self.fx_evo_branch_body_copy(
                offspring)  # copy remaining nodes in 'branch_y' ('gp.tree') to 'offspring'
            offspring = self.fx_evo_tree_prune(offspring,
        ...
        while node_count < len(
            ...
            for j in range(1, len(tree[3])):  # increment through all nodes in tourn_winner ('tree')
                ...
                if tree[5][j] == '':
                    ...
                    if tree[5][j] == 'func':
                        c_buffer = self.fx_evo_c_buffer(tree,
    ...
    def fx_evo_c_buffer(self, tree, node):
        ...
        for n in range(1, len(tree[3])):  # increment through all nodes (exclude 0) in array 'tree'
            ...
            if int(tree[4][n]) == int(tree[4][node]) and int(tree[3][n]) < int(
                ...
                if tree[8][n] != '': prior_sibling_arity = prior_sibling_arity + int(
                    tree[8][n])  # sum prior sibling arity
    ...
    def fx_evo_child_link(self, tree, node, c_buffer):
        ...
        if tree[8][node] != '':
            ...
            if int(tree[8][node]) == 0:  # if arity = 0
                ...
                print '\n\t\033[31mERROR! In fx_evo_child_link: node', node, 'has arity', tree[8][
                    node];
                self.fx_karoo_pause(0)
    ...
    def fx_evo_child_insert(self, tree, node, c_buffer):
        ...
        if int(tree[8][node]) == 0:  # if arity = 0
            print '\n\t\033[31mERROR! In fx_evo_child_insert: node', node, 'has arity 0\033[0;0m';
            self.fx_karoo_pause(0)
            ...
            print '\n\t\033[31mERROR! In fx_evo_child_insert: node', node, 'arity > 3\033[0;0m';
            self.fx_karoo_pause(0)
    ...
    def fx_test_classify(self, tree_id):
        ...
        for row in range(0, self.data_test_rows):
            result = self.fx_eval_subs(tree_id,
                                       self.data_test_dict_array[row])  # process the expression against the test data
            ...
            if result <= 0 - skew:  # test for the first class (the left-most bin)
                ...
                print '\t\033[36m data row', row, 'predicts class:\033[1m', label_pred, '(', label_true, 'label) as', result, '>', (
                                                                                                                                       self.class_labels - 2) - skew, '\033[0;0m'

            else:
                for class_label in range(1,
                                         self.class_labels - 1):  # increment through all class labels, skipping first and last

                    if (
                                class_label - 1) - skew < result <= class_label - skew:  # test for classes between first and last
                        label_pred = class_label
                        print '\t\033[36m data row', row, 'predicts class:\033[1m', label_pred, '(', label_true, 'label) as', (
                                                                                                                                  class_label - 1) - skew, '<', result, '<=', class_label - skew, '\033[0;0m'

                    else:
                        pass  # print '\t\033[36m data row', row, 'predicts no class with label', class_label, '(', label_true, ') and result', result, '\033[0;0m'

            if label_pred == '': print '\n\t\033[31mERROR! In fx_test_classify: tree', tree_id, 'failed to generate a class label prediction\033[0;0m'; self.fx_karoo_pause(
                0)
    ...
    def fx_test_regress(self, tree_id):
        ...
        for row in range(0, self.data_test_rows):
            result = self.fx_eval_subs(tree_id,
                                       self.data_test_dict_array[row])  # process the expression against the test data
            solution = round(float(self.data_test_dict_array[row]['s']),
                             self.precision)  # force 'solution' to the set number of floating points
    ...
    def fx_test_match(self, tree_id):
        ...
        print '\n\t Tree', tree_id, 'has an accuracy of:', float(self.population_b[tree_id][12][1]) / \
                                                           self.data_test_dict_array.shape[0] * 100
...
def paralell_fit_func(tree_id, start, step, finish, temp_file):
    """
    Does paralell exicution of fitness function
    :param tree_id:
    :param start:
    :param step:
    :param finish:
    ...
    :return:
    """
    # todo: send chunks to save on overhead
    self = joblib.load(temp_file)
    # determine if to go to end or to sep
    end = min([start + step, finish])
    results = sum([self.fx_fitness_eval(tree_id, row) for row in xrange(start, end)])
    return results
